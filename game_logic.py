...
# Helper function to get or initialize chat-specific data
def get_chat_data_for_id(chat_id):
    ...
    """
    return global_data["all_chat_data"].setdefault(chat_id, {
...
class DiceGame:
    ...
    def place_bet(self, user_id, username, bet_type, amount, chat_id):
        """
        ...
        if amount <= 0:
            logger.warning(f"place_bet: User {user_id} tried to bet an invalid amount ({amount}) in chat {chat_id}.")
            return False, "❌ Bet amount must be positive!"
        
        # New: Minimum bet limit check
        MIN_BET_AMOUNT = 100
        if amount < MIN_BET_AMOUNT:
            logger.warning(f"place_bet: User {user_id} tried to bet {amount} which is below minimum bet {MIN_BET_AMOUNT} in chat {chat_id}.")
            return False, f"❌ @{player['username'].replace('_', '\\_')}! Minimum bet amount is *{MIN_BET_AMOUNT}* points!"

        if player["score"] < amount:
            logger.warning(f"place_bet: User {user_id} (score: {player['score']}) tried to bet {amount} with insufficient points in chat {chat_id}.")
            return False, f"❌ @{player['username'].replace('_', '\\_')}! Insufficient points! You only have *{player['score']}* points."
        ...
        logger.info(f"place_bet: User {user_id} placed {amount} on {bet_type} for match {self.match_id} in chat {chat_id}. Total on {bet_type}: {self.bets[bet_type][user_id]}.")
    ...
    def payout(self, chat_id):
        ...
        chat_specific_data = get_chat_data_for_id(chat_id)
        stats = chat_specific_data["player_stats"] # Use chat-specific player_stats
        match_history = chat_specific_data["match_history"] # Use chat-specific match_history
